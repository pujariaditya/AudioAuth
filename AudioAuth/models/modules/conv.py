...
CONV_NORMALIZATIONS = frozenset(['none', 'weight_norm', 'spectral_norm', 'weight_standardization',
...
def get_norm_module(module: nn.Module, causal: bool = False, norm: str = 'none', **norm_kwargs) -> nn.Module:
    """Return the proper normalization module for a convolution layer.

    If *causal* is True, this will ensure the returned module is causal, or
    raise an error if the requested normalization does not support causal
    evaluation.

    Args:
        module (nn.Module): The convolution module to attach normalization to.
            Must be an instance of ``nn.modules.conv._ConvNd`` for layer-based norms.
        causal (bool): Whether the convolution operates in causal mode.
        norm (str): Normalization type. One of the values in ``CONV_NORMALIZATIONS``.
        **norm_kwargs: Additional keyword arguments forwarded to the normalization
            constructor.

    Returns:
        nn.Module: The normalization module (e.g., ``ConvLayerNorm``,
            ``nn.GroupNorm``, or ``nn.Identity`` when no normalization is needed).
    """
    assert norm in CONV_NORMALIZATIONS
    if norm == 'layer_norm':
...
def get_extra_padding_for_conv1d(x: torch.Tensor, kernel_size: int, stride: int,
    ...
    """Compute the extra padding needed so that the last convolution window is full.

    This ensures the output length after a transposed convolution can match
    the original input length exactly.

    Args:
        x (torch.Tensor): Input tensor of shape ``(..., length)``.
        kernel_size (int): Convolution kernel size.
        stride (int): Convolution stride.
        padding_total (int): Total padding already applied to the input.

    Returns:
        int: Number of extra padding samples to append at the end of the input.
    """
    length = x.shape[-1]
...
def pad_for_conv1d(x: torch.Tensor, kernel_size: int, stride: int, padding_total: int = 0):
    """Pad a 1-D input so that the last convolution window is full.

    Extra padding is added at the end. This is required to ensure that we can
    rebuild an output of the same length, as otherwise, even with padding, some
    time steps might get removed.
...
class NormConv2d(nn.Module):
    ...
    def __init__(self, *args, norm: str = 'none',
        ...
        # nn.init.kaiming_normal_(conv.weight, nonlinearity=nonlinearity, a=0.2)
        # if conv.bias is not None:
        #     conv.bias.data.zero_()
        self.conv = apply_parametrization_norm(conv, norm, **norm_kwargs)
...
class NormConvTranspose1d(nn.Module):
    ...
    def __init__(self, *args, causal: bool = False, norm: str = 'none',
        ...
        # convtr.weight.data.mul_(math.sqrt(convtr.stride[0]))
...
class NormConvTranspose2d(nn.Module):
    ...
    to provide a uniform interface across normalization approaches.
    """
    def __init__(self, *args, norm: str = 'none', nonlinearity: str = 'linear',
                 norm_kwargs: tp.Dict[str, tp.Any] = {}, **kwargs):
        super().__init__()
...
class SConv1d(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,
                ...
                pad_mode: str = 'constant', nonlinearity: str = 'linear'):
        super().__init__()
...
class SConvTranspose1d(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,
                ...
                bias: bool = True, nonlinearity: str = 'linear'):
        super().__init__()
