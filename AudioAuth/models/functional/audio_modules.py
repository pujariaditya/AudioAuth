"""
Audio transform modules.

Provides nn.Module wrappers for STDCT, MDCT, STFT, and PQMF transforms
with forward and inverse operations for use in audio processing pipelines.
"""
...
class PQMF(torch.nn.Module):
    """Pseudo Quadrature Mirror Filter bank for subband decomposition.

    Splits audio into frequency subbands using cosine-modulated filters
    designed with the Kaiser window method.
    """
    ...
    def analysis(self, x: Tensor) -> Tensor:
        """Decompose waveform into frequency subbands.

        Args:
            x: Input waveform ``[B, 1, T]`` or ``[B, T]``.

        Returns:
            Subband signals ``[B, subbands, T // subbands]``.
        """
        if x.dim() == 2:
            x = x.unsqueeze(1)
    ...
    def synthesis(self, x: Tensor) -> Tensor:
        """Reconstruct waveform from subbands.

        Args:
            x: Subband signals ``[B, subbands, T // subbands]``.
